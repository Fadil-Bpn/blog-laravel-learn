    ...
    <div class="py-4 px-4 mx-auto max-w-screen-xl lg:py-8 lg:px-0">
        ...
        <div class="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            @forelse ($posts as $post)
                <article class="p-6 bg-white rounded-lg border border-gray-200 shadow-md transform transition duration-200 hover:scale-105">
                    @if ($post->image)
                ...
                @endif
                    ...
                    <a href="/posts/{{ $post['slug'] }}">
                        <h2
                            class="hover:underline mb-2 text-2xl font-bold tracking-tight text-gray-900 ">
                            {{ $post->title }}</h2>
                    </a>
                    <p class="mb-5 font-light text-gray-500 dark:text-gray-700">{{ Str::limit(html_entity_decode(strip_tags($post->body)), 150) }}
</p>
